...
`timescale 1 ps / 1 ps
//add define
`define Wpath ".../uart_text.txt" //insert the path
...
//    localparam CLK_DIV_BAUD    = 10416;     // Should be sum above, but measurement in sim results in this value; when 230400
    ...
    reg end_sig = 1'bZ;
    integer fw;
    integer bd_counter = 0;
    localparam T_UNIT = 1000000000; //1ms
    localparam W_TIME = 50; //write time 50ms
    
    
    
    // write tx signal in char
    always@(negedge clk_baud)
    begin
        start_sig = uart_tx;
        end_sig = 1'bZ;
        if(start_sig == 1'b0)
        begin
            repeat(1)@(negedge clk_baud);
            start_sig = 1'bZ;
            while(bd_counter < 8 )
            begin
